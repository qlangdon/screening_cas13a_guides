...
#folderPathPrefix='tiled_genomes/tiled_subtype_H6N1'
...
folderStart = folderPath.split("/")[0]
topGuidesFolder = folderStart + "/top100guides_" + subPop
os.mkdir(topGuidesFolder)
topTargets = guideChunkSumDFsorted['target'][0:100].to_list()
for target in topTargets:
    targetDict = chunkDict[target]
    targetDF = pd.DataFrame(data=targetDict)
    targetAcc = guideChunkSumDFsorted[guideChunkSumDFsorted['target']==target]['accession'].to_string(index=False)
    targetStart = guideChunkSumDFsorted[guideChunkSumDFsorted['target']==target]['start'].to_string(index=False)
    targetDF.to_csv(topGuidesFolder + '/' + targetAcc + "_" + targetStart + "_" + target + "_guide_full_hits.tsv", sep="\t", index=False)
